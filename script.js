...
if (whatsappForm) {
  whatsappForm.addEventListener("submit", function (e) {
    ...
    if (!message) {
      messageField.value = "";
      messageField.placeholder = "Please enter your message";
      messageField.focus();
      return;
    }
...
const downloadLink =
  "https://dominikustripuspamahardika.github.io/sub2unlock-global/";
function startCountdown(event) {
  event.preventDefault();
  const btn = document.getElementById("countdownDownload");
  if (!btn) return;
  let countdown = 15;
  btn.textContent = `Please Wait... (${countdown})`;
...
function changeButtonText(event) {
  ...
  if (!btn) return;
  btn.textContent = "Directed to WhatsApp Admin";
  setTimeout(() => {
